...
const manualPaymentReminderLimiter = (0, express_rate_limit_1.default)({
    windowMs: 60 * 1000,
    max: 20,
    standardHeaders: true,
    legacyHeaders: false,
    message: { error: 'Too many payment reminder requests. Please try again later.' },
    keyGenerator: (req) => req.adminId || req.ip || 'unknown',
});
...
exports.adminRouter.post('/registrations/:id/remind-payment', manualPaymentReminderLimiter, async (req, res) => {
    try {
        const participant = await models_1.Participant.findById(req.params.id);
        if (!participant) {
            return res.status(404).json({ error: 'Participant not found' });
        ...
        const portalConfig = await (0, portalState_1.getPortalConfig)();
