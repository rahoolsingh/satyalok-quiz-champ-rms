...
exports.generateAdmitCardPDF = generateAdmitCardPDF;
const pdfkit_1 = __importDefault(require("pdfkit"));
const axios_1 = __importDefault(require("axios"));
const qrcode_1 = __importDefault(require("qrcode"));
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
async function generateAdmitCardPDF(data) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            doc.roundedRect(margin, margin, contentWidth, 570, 8)
                ...
                .stroke();
            // ========== HEADER SECTION ==========
            const publicAssetsDir = path_1.default.join(process.cwd(), "public", "assets");
            // Header Background
            doc.roundedRect(margin + 1.5, margin + 1.5, contentWidth - 3, 85, 6).fill(colors.bgLight);
            doc.rect(margin + 1.5, margin + 70, contentWidth - 3, 16).fill(colors.bgLight); // Flatten bottom
            ...
            const satyalokLogoPath = path_1.default.join(publicAssetsDir, "satyalok.png");
            if (fs_1.default.existsSync(satyalokLogoPath)) {
            ...
            const competitionLogoPath = path_1.default.join(publicAssetsDir, "logo.png");
            if (fs_1.default.existsSync(competitionLogoPath)) {
                doc.image(competitionLogoPath, pageWidth - margin - 75, margin + 15, { width: 55, height: 55, fit: [55, 55], valign: "center" });
            }
            // Header Text
            doc.fontSize(18)
            ...
            doc.fontSize(11)
                ...
                .font("Helvetica")
                .text(data.eventName || "QUIZ CHAMP 2026", margin, margin + 48, { width: contentWidth, align: "center" });
            // Badge for Admit Card Type
            const badgeW = 200;
            const badgeX = (pageWidth - badgeW) / 2;
            doc.roundedRect(badgeX, margin + 74, badgeW, 24, 12).fill(colors.brand);
            // junior or senior hall ticket
            doc.fontSize(10)
                .fillColor("#ffffff")
                .font("Helvetica-Bold")
                .text(data.batchType === "JUNIOR"
                ? "JUNIOR ADMIT CARD"
                : "SENIOR ADMIT CARD", badgeX, margin + 82, {
                width: badgeW,
                align: "center",
            });
            // ========== PARTICIPANT DETAILS ==========
            ...
            currentY = addDetail("Batch Category", data.batchType === "JUNIOR"
                ? "Junior (Classes 5-10)"
                : "Senior (Classes 10+)", currentY);
            currentY = addDetail("Guardian Name", data.guardianName, currentY);
            currentY = addDetail("Mobile Number", `+91 ${data.mobileNumber}`, currentY);
            // Fetch and Cache Photo Buffer
            let fetchedPhotoBuffer = null;
            if (data.photoUrl) {
                try {
                    const photoResponse = await axios_1.default.get(data.photoUrl, {
            ...
            // Main Photo Box
            doc.roundedRect(photoX, margin + 115, photoWidth, photoHeight, 6).fill(colors.bgLight);
            ...
            if (fetchedPhotoBuffer) {
                doc.save();
                doc.roundedRect(photoX, margin + 115, photoWidth, photoHeight, 6).clip();
                ...
                doc.fontSize(9)
                    .fillColor(colors.secondary)
                    .text("Paste Recent\nPassport\nPhotograph", photoX, margin + 160, { width: photoWidth, align: "center" });
            }
            // ========== EVENT DETAILS BOX ==========
            currentY = margin + 270;
            const eventBoxHeight = data.venueMapUrl ? 110 : 75;
            doc.roundedRect(margin + 20, currentY, contentWidth - 40, eventBoxHeight, 6).fill(colors.bgLight);
            doc.roundedRect(margin + 20, currentY, contentWidth - 40, eventBoxHeight, 6)
                .lineWidth(1)
                .strokeColor(colors.border)
                .stroke();
            doc.roundedRect(margin + 20, currentY, contentWidth - 40, 25, 6).fill(colors.accent);
            doc.rect(margin + 20, currentY + 15, contentWidth - 40, 10).fill(colors.accent);
            doc.fontSize(10)
                .fillColor(colors.brand)
                .font("Helvetica-Bold")
            ...
            if (data.venueMapUrl && data.venue) {
                ...
                currentY += 25;
                const mapQrDataURI = await qrcode_1.default.toDataURL(data.venueMapUrl, {
                ...
                const qrSize = 60;
                doc.image(mapQrBuffer, pageWidth - margin - qrSize - 30, currentY - 50, {
                    width: qrSize,
                    height: qrSize,
                });
            }
            ...
            doc.fontSize(8)
                .fillColor(colors.secondary)
                .text("Fold and tear along this line. Staple this portion securely with the OMR sheet.", margin, cutY + 10, { width: contentWidth, align: "center" });
            // Tear off outer box
            const tearOffMargin = 40;
            const tearOffBoxHeight = 135;
            doc.roundedRect(tearOffMargin, cutY + 25, contentWidth, tearOffBoxHeight, 8)
                .lineWidth(1.5)
                .strokeColor(colors.brand)
                .stroke();
            // Label
            doc.roundedRect(tearOffMargin + 1.5, cutY + 26.5, 150, 20, 6).fill(colors.brand);
            doc.fontSize(8)
                .font("Helvetica-Bold")
                .fillColor("#ffffff")
            ...
            const qrDataURI = await qrcode_1.default.toDataURL(qrDataPayload, {
                errorCorrectionLevel: "M",
                margin: 0,
                color: { dark: "#000000", light: "#ffffff" },
            });
            ...
            const addTearOffDetail = (label, value, y) => {
                doc.fontSize(8)
                    .font("Helvetica")
                    .fillColor(colors.secondary)
                    .text(label, toX, y);
            ...
            tearOffY += 8;
            tearOffY = addTearOffDetail("Class:", `${data.class} (${data.batchType})`, tearOffY);
            ...
            if (fetchedPhotoBuffer) {
                doc.save();
                doc.roundedRect(miniPhotoX, miniPhotoY, miniPhotoW, miniPhotoH, 4).clip();
                // Draw base image
                ...
                const holoGradient = doc.linearGradient(miniPhotoX, miniPhotoY, miniPhotoX + miniPhotoW, miniPhotoY + miniPhotoH);
                holoGradient.stop(0, "#fcd34d", 0.4); // Golden with opacity
                holoGradient.stop(1, "#ea580c", 0.5); // Orange with opacity
                doc.rect(miniPhotoX, miniPhotoY, miniPhotoW, miniPhotoH).fill(holoGradient);
                ...
                doc.roundedRect(miniPhotoX, miniPhotoY, miniPhotoW, miniPhotoH, 4)
                    .lineWidth(1)
                    .strokeColor(colors.brand)
                    .stroke();
            }
            ...
            doc.roundedRect(pageWidth - tearOffMargin - 100, cutY + 55, 85, 70, 4)
                .lineWidth(1)
                .strokeColor(colors.border)
                .stroke();
            doc.fontSize(7)
                .font("Helvetica")
                .fillColor(colors.secondary)
                .text("Invigilator Sign & Stamp", pageWidth - tearOffMargin - 100, cutY + 110, { width: 85, align: "center" });
            ...
            doc.fontSize(6)
                .font("Helvetica")
                .fillColor(colors.secondary)
                .text(`Generated on: ${genDate} | System ID: QC26-${data.rollNumber}${ipStr}`, margin, 790, { width: contentWidth, align: "right", lineBreak: false });
            doc.end();
        }
        catch (error) {
            reject(error);
...
//# sourceMappingURL=admitCardPdf.js.map