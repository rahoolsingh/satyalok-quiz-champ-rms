...
export function MobileEntry({ batchType, onSuccess, onBack }: Props) {
  ...
  return (
    <motion.div className="w-full" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.35 }}>
      ...
      <AnimatePresence>
        {alreadyRegistered && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <Link to="/track" className="font-semibold underline">View your admit card →</Link>
          </motion.div>
        )}
      ...
      <p className="text-center text-[#86868b] text-xs mt-5">
        Already registered?{' '}
        <Link to="/track" className="text-[#0066cc] font-medium hover:opacity-75">Track your registration</Link>
      </p>
    </motion.div>
  );
}
